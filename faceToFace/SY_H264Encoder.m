...
@interface SY_H264Encoder(){
    VTCompressionSessionRef _encodeSesion;
    long    _frameCount;
    dispatch_queue_t  _syEncodeQueue_video;
    int _fps;       // 帧率
    int _bitRate;   // 比特率
}
@property (nonatomic, copy) VideoEncoderCompletionBlock completionBlock;
@property (nonatomic, strong) NSData *sy_sps;
@property (nonatomic, strong) NSData *sy_pps;

@end


@implementation SY_H264Encoder

- (instancetype)init {
    if (self = [super init]) {
        _syEncodeQueue_video = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
...
// 利用系统 VideoToolbox 视频硬编码框架进行 h.264 编码
- (int)startEncodeSessionWithVideoSize:(CGSize)size framerate:(int)fps bitrate:(int)bt {
    ...
    //没完成一帧的编码后，回调VideoCompressonOutputCallback
...
- (void)encodeWithSampleBuffer:(CMSampleBufferRef )sampleBuffer timeStamp:(uint64_t)timeStamp completionBlock:(VideoEncoderCompletionBlock)completionBlock {
    ...
    dispatch_sync(_syEncodeQueue_video, ^{
...
static void VideoCompressonOutputCallback(void *userData, void *sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer) {
    ...
    SY_H264Encoder *coder = (__bridge SY_H264Encoder *)userData;
    ...
    // S PS 对于H264而言，就是编码后的第一帧，如果是读取的H264文件，就是第一个帧界定符和第二个帧界定符之间的数据的长度是4
    // P PS 就是编码后的第二帧，如果是读取的H264文件，就是第二帧界定符和第三帧界定符中间的数据长度不固定。
    if (keyFrame && !coder.sy_sps)
    {
    ...
    if (error == noErr) {
        ...
        while (offset < totalLength - lengthInfoSize) {
            ...
            SY_VideoFrame *videoFrame = [SY_VideoFrame new];
            videoFrame.timestamp = timeStamp;
            videoFrame.data = [[NSData alloc] initWithBytes:(data + offset + lengthInfoSize) length:naluLength];
            videoFrame.isKeyFrame = keyFrame;
            videoFrame.sps = coder.sy_sps;
            videoFrame.pps = coder.sy_pps;
            if (coder.delegate && [coder.delegate respondsToSelector:@selector(sy_videoEncoder_call_back_videoFrame:)]) {
                [coder.delegate sy_videoEncoder_call_back_videoFrame:videoFrame];
            }
            // 保存nalu数据到文件
