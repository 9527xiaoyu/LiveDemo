...
@interface SY_RTMP_Socket() {
    RTMP *_SY_Rtmp;
    dispatch_semaphore_t _lock;
}


@property (nonatomic, strong) SY_LiveStreamInfo *stream;
...
@property (nonatomic, strong) NSMutableArray <SY_Frame *>*frameList;
@property (nonatomic, strong) NSMutableArray <SY_Frame *>*list;
@end

@implementation SY_RTMP_Socket
...
- (void) start {
    dispatch_async(self.socketQueue, ^{
        ...
        [self SY_RTMP264_Connect:(char *)[self.stream.url cStringUsingEncoding:NSASCIIStringEncoding]];
    });
}
...
- (void) stop {
    dispatch_async(self.socketQueue, ^{
        if (_SY_Rtmp != NULL) {  // 如果有 remp 对象
            RTMP_Close(_SY_Rtmp);    // 关闭
            RTMP_Free(_SY_Rtmp);     // 释放
            _SY_Rtmp = NULL;
        }
        [self resetParameter];
    });
...
-(NSInteger)SY_RTMP264_Connect:(char *)push_url{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:SY_LivePending];
    }
    ...
    _SY_Rtmp = RTMP_Alloc();       // 用于创建一个RTMP会话的句柄。
    RTMP_Init(_SY_Rtmp); // 初始化句柄
    
    // 设置会话参数 URL
    if (RTMP_SetupURL(_SY_Rtmp, push_url) < 0) {
        goto Failed;     // 程序跳转，叫做goto，goto可以跳到程序的任意地方
    }
    
    // 设置发布流, 这个函数必须在连接前使用, 否则无限
    RTMP_EnableWrite(_SY_Rtmp);
    // 在几秒钟内连接超时
    _SY_Rtmp->Link.timeout = RTMP_RECEIVE_TIMEOUT;
    
    // 连接服务器
    if (RTMP_Connect(_SY_Rtmp, NULL) < 0) {
        goto Failed;
    }
    
    // 连接流
    if (RTMP_ConnectStream(_SY_Rtmp, 0) < 0) {
        goto Failed;
    }
    
    if(self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]){
        [self.delegate socketStatus:self status:SY_LiveStart];
    }
...
Failed:
    ...
    [self resetParameter];
    if(self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]){
        [self.delegate socketStatus:self status:SY_LiveError];
    }
    return -1;
}
...
- (void)sendFrame {
    if (!self.isSending && self.list.count > 0) {
        self.isSending = YES;
        if (!_isConnected || _isReconnecting || _isConnecting || !_SY_Rtmp) return;
        SY_Frame *frame = [self popFirstObject];
        if ([frame isKindOfClass:[SY_VideoFrame class]]) {  // RTMP 发送视频
            SY_VideoFrame *videoFrame = (SY_VideoFrame *)frame;
            ...
            SY_AudioFrame *audioFrame = (SY_AudioFrame *)frame;
...
// 发送视频包
- (void)sendVideo:(SY_VideoFrame*)frame{
...
#pragma mark 发送语音
// 发送语音包头
- (void)sendAudioHeader:(SY_AudioFrame*)audioFrame{
...
-(NSInteger) sendPacket:(unsigned int)nPacketType data:(unsigned char *)data size:(NSInteger) size nTimestamp:(uint64_t) nTimestamp{
    ...
    if(_SY_Rtmp) rtmp_pack.m_nInfoField2 = _SY_Rtmp->m_stream_id;
    ...
    NSInteger nRet;
    if (RTMP_IsConnected(_SY_Rtmp)){
        int success = RTMP_SendPacket(_SY_Rtmp,&rtmp_pack,0);    // true 为放进发送队列, false 不放进发送队列直接发送
        if(success){
            self.isSending = NO;
            [self sendFrame];
        }
...
- (void)appendObject:(SY_Frame *)frame {
    if (!frame) {
        return;
    }
    ...
    if(self.frameList.count < defaultFrameListMaxCount){
        ...
        /// 添加至缓冲区
        SY_Frame *firstFrame = [self.frameList SY_PopFirstObject];
        if(firstFrame) [self.list addObject:firstFrame];
    }
    dispatch_semaphore_signal(_lock);
}

- (SY_Frame *)popFirstObject{
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
    SY_Frame *firstFrame = [self.list SY_PopFirstObject];
    dispatch_semaphore_signal(_lock);
    return firstFrame;
}
...
NSInteger frameDataCompare(id obj1, id obj2, void *context){
    SY_Frame * frame1 = (SY_Frame*) obj1;
    SY_Frame *frame2 = (SY_Frame*) obj2;
...
- (void)removeExpireFrame{
    ...
    SY_Frame *firstIFrame = [self firstIFrame];
    if(firstIFrame){
...
- (NSArray*)expirePFrames{
    ...
    for(NSInteger index = 0;index < self.list.count;index++){
        SY_Frame *frame = [self.list objectAtIndex:index];
        if([frame isKindOfClass:[SY_VideoFrame class]]){
            SY_VideoFrame *videoFrame = (SY_VideoFrame *)frame;
...
- (SY_Frame *)firstIFrame{
    for(NSInteger index = 0;index < self.list.count;index++){
        SY_Frame *frame = [self.list objectAtIndex:index];
        if([frame isKindOfClass:[SY_VideoFrame class]] && ((SY_VideoFrame*)frame).isKeyFrame){
            return frame;
        }
    }
    return nil;
